report 50009 "GXL Auto Invoice Non-EDI POs"
{
    /*
        Change log:
    ...
    */
    ...
    Caption = 'Auto Invoice Non-EDI POs';
    ProcessingOnly = true;
    UsageCategory = Tasks;
    ApplicationArea = All;
    dataset
    ...
    begin
        ...
        //ERP-397 +
        ClearLastError();
        if AutoProcessPO.IsAutoInvoiceDisabled(PH) then
            exit;
        if not AutoProcessPO.IsNonEDIPurchaseOrder(PH) then
            exit;
        //ERP-397 -
        ...
        //ERP-397 +
        if not Ok then
            if PH.Get(PH."Document Type", PH."No.") then begin
                PH."GXL Auto Invoice Error Msg" := CopyStr(GetLastErrorText(), 1, MaxStrLen(PH."GXL Auto Invoice Error Msg"));
                PH.Modify();
            end;
        //ERP-397 -
    end;
