...
report 50033 "GXL Purch Order XD"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/PurchOrderXD.rdl';
    Caption = 'Purch Order XD';             // 001 07.07.2025 BY HP2-Sprint2-Changes
        ...
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    // >> 001 07.07.2025 BY HP2-Sprint2-Changes
                    dataitem(XDSTO; Integer)
                    {
                        DataItemTableView = SORTING(Number) ORDER(Ascending) WHERE(Number = CONST(1));

                        trigger OnPreDataItem()
                        begin
                            IF "Purchase Header"."GXL Source of Supply" IN ["Purchase Header"."GXL Source of Supply"::SD, "Purchase Header"."GXL Source of Supply"::WH] THEN BEGIN
                                CurrReport.BREAK;
                            END;
                        end;
                    }
                    ...
                    {
                        DataItemTableView = SORTING("JDA PO No.") WHERE("GXL Source of Supply" = FILTER(XD | FT));
                        RequestFilterHeadingML = ENU = 'Transfer Order', ENA = 'Transfer Order';
                        RequestFilterFields = "No.", "Transfer-from Code", "Transfer-to Code";

                        column(PO_TranHeader; "Transfer Header"."JDA PO No.")
                        {
                        }
                        ...
                        column(TransferTo; "Transfer Header"."Transfer-to Code")
                        {
                        }
                        ...
                        trigger OnPreDataItem()
                        begin
                            SETFILTER("JDA PO No.", "Purchase Header"."GXL JDA Load ID");
                            IF COUNT = 0 THEN
                                CurrReport.SKIP;
                        end;
                        ...
                        begin
                            ...
                            recTransLine.SETFILTER("JDA PO No.", "Purchase Header"."GXL JDA Load ID");
                            recTransLine.SETFILTER(Quantity, '>0');
                            IF NOT recTransLine.FINDFIRST THEN
                                CurrReport.SKIP;
                        end;
                    }
                    dataitem("Transfer Line"; "Transfer Line")
                    {
                        DataItemTableView = SORTING("Document No.", "Line No.") WHERE("Derived From Line No." = CONST(0), Quantity = FILTER(> 0));
                        ...
                        column(VendReorderNo_TransLine; "Transfer Line"."Vendor Reorder No.")
                        {
                        }
                        column(Barcode_TransLine; "Transfer Line"."Cross-Reference No.")          // do not create ask kartik sir
                        {
                        }
                        column(CtnQty_TransLine; "Transfer Line"."Carton-Qty")
                        {
                        }
                        column(UnitCost_TransLine; "Transfer Line"."GXL Unit Cost")
                        {
                        }
                        column(TotalCost_TransLine; "Transfer Line"."GXL Total Cost")
                        {
                        }
                        column(OM_TransLine; Item."GXL Order Multiple (OM)")
                        {
                        }
                        column(OP_TransLine; Item."GXL Order Pack (OP)")
                        {
                        }
                        column(TotalAmtInclVAT_TransLine; TransTotalAmtIncGST)
                        {
                            AutoFormatType = 1;
                        }
                        column(TotalAmt_TransLine; TransTotalAmt)
                        {
                            AutoFormatType = 1;
                        }
                        ...
                        begin
                            ...
                            "Transfer Line"."Vendor Reorder No." := GetVendorReorderNo("Item No.");
                            "Transfer Line"."Cross-Reference No." := GetBarCode("Item No.");
                            TransTotalAmt += "Transfer Line"."Gxl Total Cost";
                            TransTotalAmtIncGST := TransTotalAmt * 1.1;
                        end;
                    }
                    // << 001 07.07.2025 BY HP2-Sprint2-Changes
                }
            ...
            begin
                ...
                // << 001
                FooterTerms[1] := Text50001;
                FooterTerms[2] := STRSUBSTNO(Text50002, CompanyInfo."E-Mail", CompanyInfo."Phone No.");
                // << 001 07.07.2025 BY HP2-Sprint2-Changes
                if not IsReportInPreviewMode then
    ...
    begin
        GLSetup.Get;
        CompanyInfo.Get;
        CompanyInfo.CALCFIELDS(Picture);     // 001 07.07.2025 BY HP2-Sprint2-Changes
        PurchSetup.Get;
    ...
    var
        ...
        VATDiscAmtCaptionLbl: Label 'Payment Discount on GST';
        LineDimsCaptionLbl: Label 'Line Dimensions';
        VATPercentCaptionLbl: Label 'GST %';
        VATBaseCaptionLbl: Label 'GST Base';
        VATAmtCaptionLbl: Label 'GST Amount';
        VATAmtSpecCaptionLbl: Label 'GST Amount Specification';
        VATIdentCaptionLbl: Label 'GST Identifier';
        ...
        PrepmtVATAmtSpecCaptionLbl: Label 'Prepayment GST Amount Specification';
        ...
        FooterTerms: ARRAY[6] OF Text;
        Text50001: Label 'Orders not delivered by the Delivery Date will be cancelled unless advised 3 days prior to the Delivery Date. Please email PBOrders@Petbarn.com.au to advise of any issues.';
        Text50002: Label 'Orders are to be Booked for Delivery within 48 hours of the Delivery Date. Booking Requests are to be emailed to %1 or phoned through to %2';
        Item: Record 27;
        TransTotalAmt: Decimal;
        TransTotalAmtIncGST: Decimal;
    // << 001 07.07.2025 BY HP2-Sprint2-Changes
    ...
    // >> 001 07.07.2025 BY HP2-Sprint2-Changes
    procedure GetVendorReorderNo(ItemNo: Code[20]): Code[20];
    VAR
        ItemCrossRef: Record "Item Reference";
    BEGIN
        // >> PSSC.00
        ItemCrossRef.RESET;
        ItemCrossRef.SETRANGE(ItemCrossRef."Item No.", ItemNo);
        ItemCrossRef.SETRANGE(ItemCrossRef."Reference Type", ItemCrossRef."Reference Type"::Vendor);
    ...
    procedure GetBarCode(ItemNo: Code[20]): Code[20];
    VAR
        ItemCrossRef: Record "Item Reference";
    ...
    BEGIN
        ...
        ItemCrossRef.RESET;
        ItemCrossRef.SETRANGE(ItemCrossRef."Item No.", ItemNo);
        ItemCrossRef.SETRANGE(ItemCrossRef."Reference Type", ItemCrossRef."Reference Type"::"Bar Code");
        IF ItemCrossRef.FINDFIRST() THEN
            EXIT(ItemCrossRef."Reference No.");
