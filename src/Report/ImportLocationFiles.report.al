// 005 03.10.2024 LCB-258 Update error if the qty is not matching
// 004 12.07.2024 LCB-291 Update Reason code while posting item jnl lines
// 003 13.06.2024 KDU LCB-501 https://petbarnjira.atlassian.net/browse/LCB-501
// 002 13.06.2024  SKY  LCB-524 https://petbarnjira.atlassian.net/browse/LCB-524 
// 001 15.05.2024 KDU LCB-298 https://petbarnjira.atlassian.net/browse/LCB-298 - Fix the posting date error
///<Summary>
...
///</Summary>
report 50351 "GXL Import Location Files"
{
                        ...
                        IF PurchaseLine.GET(PurchaseLine."Document Type"::Order, PurchaseHeader."No.", WHMessageLines."Line No.") THEN BEGIN
                            ...
                            END ELSE BEGIN
                                ...
                                if (PurchaseLine.Quantity < WHMessageLines."Qty. To Receive") then begin
                                    ...
                                    //PurchaseLine.Validate(Quantity, WHMessageLines."Qty. To Receive"); // >> 005 <<
    ...
    var
        ...
        OriginalStatus: Option Open,Released,"Pending Approval","Pending Prepayment"; // >> 002 << 
    begin
        COMMIT();
        ...
        IF PurchaseHeader.FINDFIRST() THEN BEGIN
            ...
            IF PurchaseHeader."GXL Order Status" <> PurchaseHeader."GXL Order Status"::Closed THEN BEGIN
                ...
                COMMIT();
                // >> 002 
                OriginalStatus := PurchaseHeader.Status;
                ...
                PurchaseHeader.Status := OriginalStatus; // >> 002 <<    
    ...
    var
        ...
        //IntegrationSetupL: Record "GXL Integration Setup"; // >> 001 <<
        ItemJnlPostBatch: Codeunit "Item Jnl.-Post Batch";
        GXLMiscUtilities: Codeunit "GXL Misc. Utilities";// >> 001 <<
        LineNo: Integer;
    begin
        //ERP-301 - 3PL Transfer +
        //Added param FromTransfer
        //IntegrationSetupL.Get(); // >> 001 <<
        ...
        // >> 001
        // if not IntegrationSetupL."TOR Auto Decrease Enable" then
        //     RecWHMessageLines.SetRange("Entry Type", 'POSITIVE ADJMT');
        // << 001
        IF RecWHMessageLines.FindSet() THEN BEGIN
            ...
            REPEAT
                ...
                END;
                // >> 001
                if (_recItemJournal."Entry Type" = _recItemJournal."Entry Type"::"Negative Adjmt.")
                    and (StrPos(_recItemJournal."Document No.", 'TOR') > 0) and (StrPos(_recItemJournal."Document No.", 'DECREASE') > 0)
                then
                    _recItemJournal."Posting Date" := GXLMiscUtilities.GetCurrentInventoryPeriod(_recItemJournal."Posting Date");
                // << 001
                ...
                _recItemJournal."Reason Code" := RecWHMessageLines."BC Reason Code";

                IF (NOT RecReason.GET(RecWHMessageLines."BC Reason Code")) OR (RecWHMessageLines."BC Reason Code" = '') THEN
                    _recItemJournal."Reason Code" := IntegrationSetup."Default STK Adj. Reason Code";
                // << 004
    ...
    // >> 001
    local procedure InvPeriodIsClosedFor(DateP: Date): Boolean
    var
    ...
    begin
        ...
        InventoryPeriodL.SetRange(Closed, false);
        InventoryPeriodL.SetFilter("Ending Date", '<=%1', DateP);
        exit(InventoryPeriodL.IsEmpty);
    end;

    local procedure FirstDayOfCurrentInvPeriod(): Date
    var
        InventoryPeriodL: Record "Inventory Period";
        InvEndDateL: Date;
    begin
