// 001 04.07.2025 MAY HP2-Sprint2-Changes
report 50037 "International Purchase Order"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/InternationalPurchOrder1.rdl';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            // 001 04.07.2025 MAY HP2-Sprint2-Changes
            DataItemTableView = SORTING("Document Type", "No.") WHERE("Document Type" = CONST(Order), "GXL Domestic Order" = filter(False));
            ...
            // 001 04.07.2025 MAY HP2-Sprint2-Changes
            column(CompContact; CompContact)
            {
            }
            // 001 04.07.2025 MAY HP2-Sprint2-Changes
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(Terms1; Terms[1])
                    {
                    }
                    column(Terms2; Terms[2])
                    {
                    }
                    column(Terms3; Terms[3])
                    {
                    }
                    column(Terms4; Terms[4])
                    {
                    }
                    column(Terms5; Terms[5])
                    {
                    }
                    column(Terms6; Terms[6])
                    {
                    }
                    column(FooterTerms1; FooterTerms[1])
                    {
                    }
                    column(FooterTerms2; FooterTerms[2])
                    {
                    }
                    column(LocPhoneNo; Location."Phone No.")
                    {
                    }
                    column(LocFaxNo; Location."Fax No.")
                    {
                    }
                    column(PhoneNo; BuyFromVendor."Phone No.")
                    {
                    }
                    column(FaxNo; BuyFromVendor."Telex No.")
                    {
                    }
                    column(Email; BuyFromVendor."E-Mail")
                    {
                    }
                    ...
                    column(SelltoCustNo_PurchHdrCaption1; "Purchase Header".FIELDCAPTION("Sell-to Customer No."))
                    {
                    }
                    ...
                    // 001 04.07.2025 MAY HP2-Sprint2-Changes
                    column(DeliveryDate; FORMAT("Purchase Header"."Expected Receipt Date", 0, '<Day>-<Month Text,3>-<Year>'))
                    {
                    }
                    column(VendorShipDate; FORMAT("Purchase Header"."GXL Vendor Shipment Date", 0, '<Day>-<Month Text,3>-<Year>'))
                    {
                    }
                    ...
                    column(ShipToContact; "Purchase Header"."Ship-to Contact")
                    {
                    }
                    ...
                    column(DepartPort; DepartPort)
                    {
                    }
                    column(ArrivalPort; ArrivalPort)
                    {
                    }
                    column(CountryName; RecCountry.Name)
                    {
                    }
                    column(ExpectedShipDate; FORMAT("Purchase Header"."GXL Expected Shipment Date", 0, '<Day>-<Month Text,3>-<Year>'))
                    {
                    }
                    column(TotalPack; "Purchase Header"."GXL Total Order Qty")
                    {
                    }
                    column(Incoterms; "Purchase Header"."GXL Incoterms Code")
                    {
                    }
                    column(TotalWeight; "Purchase Header"."GXL Total Weight")
                    {
                    }
                    column(TotalCubage; "Purchase Header"."GXL Total Cubage")
                    {
                    }
                    column(VendorContact; PlannerName)
                    {
                    }
                    // 001 04.07.2025 MAY HP2-Sprint2-Changes
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        // 001 04.07.2025 MAY HP2-Sprint2-Changes
                        column(VendReorderNo; "Purchase Line"."GXL Vendor Reorder No.")
                        {
                        }
                        column(GTIN; "Purchase Line"."Item Reference No.")
                        {
                        }
                        column(OM; Item."GXL Order Multiple (OM)")
                        {
                        }
                        column(OP; Item."GXL Order Pack (OP)")
                        {
                        }
                        column(CTNQty; "Purchase Line"."GXL Carton-Qty")
                        {
                        }
                        // 001 04.07.2025 MAY HP2-Sprint2-Changes
                        ...
                        begin
                            ...
                            //PSSC.00>>
                            Item.RESET;
                            IF (TempPurchaseLine.Type = TempPurchaseLine.Type::Item) AND (TempPurchaseLine."No." <> '') THEN
                ...
                begin
                    ...
                    OutputNo := 0;
                    //SetFilter("domestic order", No);// 001 04.07.2025 MAY HP2-Sprint2-sChanges
                end;
            }
            ...
            // 001 04.07.2025 MAY HP2-Sprint2-Changes
            var
                JDAMgmt: Codeunit "GXL JDA Management";
                Vend: Record 23;
                CompShortName: Text[20];
                AccPayEmail: Text[80];
            // 001 04.07.2025 MAY HP2-Sprint2-Changes
            begin
                ...
                IF Vend.GET("Buy-from Vendor No.") THEN
                    ...
                    IF Vend."PO Address from Company" > '' THEN begin
                        CompanyInfo.CHANGECOMPANY(Vend."PO Address from Company");
                        CompanyInfo.Get();
                ...
                RecCountry.reset;   // 001 04.07.2025 MAY HP2-Sprint2-Changes
                ...
                POType := '';
                POType := JDAMgmt.GetPOType("Purchase Header");
                ...
                // >> 001
                //Terms[1] :='Please be aware Petbarn does not accept backorders of any product';
                //Terms[2] :='Items unsupplied will be re-ordered as required';
                //Terms[3] :='Fulfilment of this Purchase Order constitutes acceptance of the Petbarn Cost Prices';
                //Terms[4] :='Petbarn does not accept charges for Freight, Packaging, Handling or Fuel Levies';
                //Terms[5] :='Any Cost Price queries are to be directed to the Petbarn Product Team';
                //Terms[6] :='All invoices must be forwarded to accountspayable@Petbarn.com.au';

                IF CompanyInfo."Short Name" > '' THEN
                    CompShortName := CompanyInfo."Short Name"
                ELSE
                    CompShortName := COPYSTR(CompanyInfo.Name, 1, STRPOS(CompanyInfo.Name, ' ') - 1);

                CompContact := UPPERCASE(CompShortName) + ' CONTACT';

                IF CompanyInfo."Accounts Payable E-Mail" > '' THEN
                    AccPayEmail := CompanyInfo."Accounts Payable E-Mail"
                ELSE
                    AccPayEmail := 'accountspayable@Petbarn.com.au';

                Terms[1] := STRSUBSTNO('Please be aware %1 does not accept backorders of any product.', CompShortName);
                Terms[2] := 'Items unsupplied will be re-ordered as required.';
                Terms[3] := STRSUBSTNO('Fulfilment of this Purchase Order constitutes acceptance of the %1 Cost Prices.', CompShortName);
                Terms[4] := STRSUBSTNO('%1 does not accept charges for Freight, Packaging, Handling or Fuel Levies.', CompShortName);
                Terms[5] := STRSUBSTNO('Any Cost Price queries are to be directed to the %1 Product Team.', CompShortName);
                Terms[6] := STRSUBSTNO('All invoices must be forwarded to %1', AccPayEmail);
                // << 001

                "Purchase Header".CALCFIELDS("GXL Total Order Qty");
                //>> MCS1.49 (IPP-12)
                "Purchase Header".CALCFIELDS("GXL Total Weight");
                "Purchase Header".CALCFIELDS("GXL Total Cubage");
                //<< MCS1.49 (IPP-12)

                //ooterTerms[1] := Text50001;
                //>> MCS1.57
                FooterTerms[1] := STRSUBSTNO(Text50001, CompanyInfo."E-Mail", CompanyInfo."Phone No.");
                PlannerName := '';
                IF BuyFromVendor."GXL Supplier Planner" <> '' THEN
                    IF PlannerSetup.GET(BuyFromVendor."GXL Supplier Planner") THEN
                        PlannerName := PlannerSetup.Name;

                FooterTerms[1] := STRSUBSTNO(Text50002, CompanyInfo."E-Mail", CompanyInfo."Phone No.");
                //<< MCS1.57
                RecCountry.RESET;
                IF "Purchase Header"."Buy-from Country/Region Code" <> '' THEN
                    RecCountry.GET("Purchase Header"."Buy-from Country/Region Code");

                DepartPort := '';
                ArrivalPort := '';
                RecPort.RESET;
                IF RecPort.GET("Purchase Header"."GXL Arrival Port") THEN
                    ArrivalPort := RecPort.Description;
                RecPort.RESET;
                IF RecPort.GET("Purchase Header"."GXL Departure Port") THEN
                    DepartPort := RecPort.Description;
                //>>001
                if not IsReportInPreviewMode() then
    ...
    begin
        GLSetup.Get();
        CompanyInfo.Get();
        CompanyInfo.CALCFIELDS(Picture);// 001 04.07.2025 MAY HP2-Sprint2-Changes   
        PurchSetup.Get();
    ...
    var
        ...
        // 001 04.07.2025 MAY HP2-Sprint2-Changes
        BuyFromVendor: Record 23;
        Location: Record 14;
        Terms: ARRAY[6] OF Text;
        Item: Record 27;
        FooterTerms: ARRAY[2] OF Text;
        Text50001: Label 'Orders are to be Booked via DHL 2 weeks prior to the Ship Date. Booking Requests are to be emailed to %1 or phoned through to %2';
        POType: Text;
        RecCountry: Record 9;
        RecPort: Record RecPort;
        DepartPort: Text;
        ArrivalPort: Text;
        Text50002: Label 'Orders are to be booked via Geodis Logistics 2 weeks prior to the Ship Date using the Petbarn purchase order reference number. Booking requests and all relevant shipping paperwork are to be emailed to bu.green.ff.au@geodis.com and Logistics.Intl@Petbarn.com.au. All shipments originating from China, Thailand, Malaysia and USA need to be accompanied by a Free Trade Agreement/Certificate of Origin(CoO) within 7 days, after vessels depart. Additional charges resulting from non-compliance will be charged back to the suppliers at cost.';
        //"-- JM.IPP-33"1101214001 : Integer;
        PlannerSetup: Record 50016;
        PlannerName: Text[50];
        CompContact: Text;
        Text50003: Label 'This Order is subject to the  Greencross Supplier Terms and Conditions  available at https://greencrosslimited.sharepoint.com/sites/GreencrossLimitedSupplierSharepoint. By accepting this Order, the Supplier agrees to be bound by that agreement.';
    // 001 04.07.2025 MAY HP2-Sprint2-Changes
